...
def ti_summary_rec(tisr_items, tisr_idx, tisr_pending, tisr_completed, tisr_cancelled):
    for tisr_item in tisr_items:
        ...
        if tisr_status == "pending":
            ...
            tisr_cancelled += 1
    return {
        "pending": tisr_pending,
        "completed": tisr_completed,
        "cancelled": tisr_cancelled,
    }


def ti_create_transfer(
    tic_product_id,
    tic_source_warehouse_id,
    tic_dest_warehouse_id,
    tic_quantity,
    tic_created_by,
):
