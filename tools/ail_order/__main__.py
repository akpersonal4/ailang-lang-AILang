...
from tools.ail_order.reporter import generate_markdown_report, generate_json_report, write_reports
...
def main() -> int:
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Output only JSON (machine readable)"
    )
    parser.add_argument(
        "--fix",
        action="store_true",
    ...
    if isinstance(analysis, FileAnalysis):
        has_errors = any(f.severity == analysis.findings[0].severity if analysis.findings else None
