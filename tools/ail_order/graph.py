...
def detect_cycles(functions: list[FunctionInfo]) -> tuple[list[list[str]], list[OrderingFinding]]:
...
def detect_unreachable(functions: list[FunctionInfo]) -> list[str]:
    ...
    unreachable = [fn.name for fn in functions if fn.name not in reachable and fn.name != "main"]
    return unreachable
    ...
    for unreachable_fn in file_analysis.unreachable:
        ...
        for fn in functions:
            if fn.name == unreachable_fn:
                file_analysis.findings.append(OrderingFinding(
