...
from core.helpers import helpers_find_in_list, helpers_get_map_value_safe, helpers_generate_id, helpers_current_timestamp


def reorder_get_level(product_id):
    return helpers_find_in_list(storage_list("reorder_levels"), "product_id", product_id)
...
def reorder_set_level(product_id, min_level, max_level, reorder_qty):
    ...
    return storage_update("reorder_levels", existing["id"], {
        "min_level": min_level, "max_level": max_level,
        "reorder_qty": reorder_qty, "updated_at": now,
    })
...
def reorder_list_needed():
    return [r for r in storage_list("reorder_levels") if r.get("current_stock", 0) > r.get("min_level", 0)]
