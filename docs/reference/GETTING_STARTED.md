...
AILang deliberately introduces specific architectural constraints to achieve **100% determinism**, **eliminate side effects**, and **optimize for AI code generation**. If you are coming from Python, JavaScript, or C++, these decisions are intentional:

### 1. Recursion-Only Workflow (No Loops)
AILang has no `while` or `for` loops. All iterative operations must be expressed via recursion.
* **Why?** Mutable, imperative loops are a primary source of state bugs, off-by-one errors, and complex side-effects that are difficult for static analysis (and AI systems) to reason about. By restricting iteration to recursion, every iteration is a clean, mathematically provable state transition where arguments define the state at each step. This ensures complete determinism and allows AILang programs to be verified with mathematical precision.
* **Example:**
  ```ail
  // Recursively process task list
  fn print_tasks_recursive(tasks, idx) {
      if (idx >= list.len(tasks)) {
          return 0; // base case: we've reached the end of the list
      }
      let task = list.get(tasks, idx);
      print_task(task);
      return print_tasks_recursive(tasks, idx + 1); // recursive step
  }
  ```

### 2. Mandatory Return Expressions (`return <expr>;`)
Every `return` statement in AILang must return an explicit expression (e.g., `return 0;`, `return true;`, `return "";`). Empty `return;` statements are syntax errors.
* **Why?** In AILang, functions are designed as mathematical mappings from inputs to outputs. A function with an implicit or absent return value introduces non-determinism and complicates the type checker. Requiring every return statement to have an explicit value guarantees that the return type of every function can be statically and deterministically inferred.
* **Tip:** If a function does not need to return a meaningful value, return `0`, `true`, or a success code (e.g., `return 0;`).

### 3. No User-Defined Types (Maps and Lists Only)
There are no `class`, `struct`, or custom type declarations in AILang. All structured data is represented using primitive `map` (key-value) and `list` (dynamic array) types.
* **Why?** Custom type systems introduce significant structural complexity, boilerplate, and compiler overhead (e.g., class hierarchies, inheritance, interfaces). Since LLMs and AI agents excel at working with raw JSON-like structures, AILang relies entirely on `map` and `list`. This maps perfectly to JSON serialization/deserialization, eliminates impedance mismatches, and allows AI models to construct, mutate, and inspect data structures with zero boilerplate.
* **Example:**
  ```ail
  import map;
  
  fn create_task(id, title) {
      let task = map.new();
      map.set(task, "id", id);
