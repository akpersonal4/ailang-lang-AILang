...
Each benchmark was evaluated on whether it:
1. **Compiles** — AILang compiler accepts the program without errors
2. **Runs correctly** — Produces expected output matching the specification
3. **Is maintainable** — Code is readable and organized (qualitative)
4. **Is performant** — Execution time is acceptable for the problem domain

### 13.2 Scoring Scale

| Rating | Meaning |
|:------:|---------|
| 9–10 | Production-ready. Compiles and runs correctly on first attempt. |
| 7–8 | Minor friction. Requires ≤2 iterations. Suitable for production with caveats. |
| 5–6 | Significant friction. Requires multiple iterations. Feasible but costly. |
| 3–4 | Major limitations. Correct but impractical for production. |
| 1–2 | Not suitable. Cannot reliably implement the required functionality. |

### 13.3 Evidence Bars (per GOVERNANCE.md)

| Bar | Threshold | Used For |
|:---:|:---------:|----------|
| **Single report** | 1 benchmark | Flag for investigation. Not sufficient for language changes. |
| **Replicated** | 2 benchmarks | Minimum bar for stdlib additions. |
| **Strong** | 3+ benchmarks | Sufficient for stdlib changes. Required for language evolution. |
| **Universal** | 6+ benchmarks | Core language characteristic. Changes require supermajority governance vote. |
...
### 15.2 What AILang Does Not Do Well
