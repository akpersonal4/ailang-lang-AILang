...
| 4 | `for` loop / iterators | Language | grep, diff, file_organizer, dep_graph | ⚠️ Conditional | Implemented as experimental `for item in collection { body }` in v0.9.0 (M25/M26/M27) with `--experimental-loops` flag. Lowers to recursive helper functions. Capture semantics added in M27. Pending v1.0 promotion decision after evaluation. | 0.9.0 |
...
| 7 | Float literal syntax `3.14` | Language | scientific_calculator, grade_calculator, bmi_calculator (Engineering Olympics P1 gap) | ✅ Implemented v0.2.0 | Grammar extended: `number_literal = digit, { digit }, [ ".", digit, { digit } ]`. Lexer, IR builder, type checker already had dormant float support. Engineering Olympics P1 gap closed. | 0.2.0 |
...
- [Product Roadmap](../../roadmap/PRODUCT_ROADMAP.md) — Forward-looking feature planning
