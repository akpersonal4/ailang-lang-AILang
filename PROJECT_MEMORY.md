...
| **Result** | RC verdict READY FOR PUBLICATION. v1.1.22 publication FAILED (artifacts dropped the de-facto-shipped `stdlib/test.ail` → `import test` MOD003). Corrective **v1.1.23 published** (commit `0ae2b77`, tag `v1.1.23`): module restored to wheel/sdist, fresh-PyPI consumer battery 22/22 incl. `import test`/`test.expect`/testgen, all release gates passed (`AILANG_A101_V1_1_23_CORRECTIVE_RELEASE_REPORT.md`). |
| **Lessons** | (1) A stale regular install of an old version in the dev venv can shadow current source in subprocesses that don't pin PYTHONPATH — upgrade the venv install when validating new builtins. (2) Untracked files under packaged directories silently leak into wheels/sdists AND can become de-facto public API: audit artifact contents against `git ls-files` before every release, and treat previously-shipped accidental files as compatibility surface that must be tracked or deliberately dropped with a migration note. (3) PowerShell 5.1 `-Encoding utf8` writes BOMs that break lexers/JSON parsers — author test fixtures via Python `[IO.File]`/`write_bytes`. (4) Full-suite totals collected under a stale-duplicate site-packages are not comparable across environment changes — verify collection counts when the install changes. |
