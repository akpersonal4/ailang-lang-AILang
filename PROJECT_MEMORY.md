...
| **Result** | RC verdict READY FOR PUBLICATION. Publication to PyPI/GitHub executed but final verdict **PUBLICATION FAILED**: v1.1.22 artifacts dropped `stdlib/test.ail` (accidentally shipped in prior wheels, never tracked) → `import test` / `test.expect` MOD003 for installed users; repo fixed post-tag (commit `e1406b8`), remediation release pending approval. |
